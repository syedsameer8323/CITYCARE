...
def update_status2():
    ...
    # Get old staff + status
    cur.execute(
        "SELECT staff_id, status FROM complaints WHERE complaint_id=%s",
        (cid,)
    )
    old = cur.fetchone()
    ...
    if sid:
        cur.execute(
            f"""UPDATE complaints 
                SET status=%s, assigned_staff=%s, staff_id=%s{rc}
    ...
    # =========================
    # FIX STAFF LOAD LOGIC ✅
    # =========================

    # 1️⃣ If resolved → decrease load
    if sts == "resolved":
        target_sid = sid or old_staff_id
        if target_sid:
            cur.execute("""
    ...
    # 2️⃣ If newly assigned (different staff)
    elif sid and sid != old_staff_id:
        cur.execute("""
            UPDATE staff_accounts
    ...
    cur.execute("""
        SELECT complaint_id, user_name, userid, phone_number, category,
               location, adddress, waste_image, status, severity,
               priority_score, assigned_staff, created_at
        FROM complaints
        ORDER BY priority_score DESC
    """)
    values = cur.fetchall()

    cur.execute("SELECT id, name, current_load FROM staff_accounts WHERE available=1")
    staff_list = cur.fetchall()

    con.close()

    return render_template(
        "view_complaints2.html",
        msg="Status updated!",
        rawdata=values,
        staff_list=staff_list
    )
...
def admin_feedback():
    ...
    row = cur.fetchone()
    ar = list(row.values())[0] if row else None
...
def complaint_update(cid):
    ...
    return render_template(
        "complaint_update.html",   # also fixed typo here
        id=cid,
...
def staff_update_complaint(complaint_id):
    ...
    cur.execute(
        """SELECT * FROM complaints
           WHERE complaint_id = %s AND staff_id = %s""",
        (complaint_id, staff_id)
    )
    complaint = cur.fetchone()
